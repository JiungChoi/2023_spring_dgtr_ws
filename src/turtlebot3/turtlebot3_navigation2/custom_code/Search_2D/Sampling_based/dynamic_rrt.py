...
from Search_2D.Sampling_based import env, plotting, utils
...
class DynamicRrt:
    def __init__(self, s_start, s_goal, step_len, goal_sample_rate, waypoint_sample_rate, iter_max, cost_map):
        s_start = (20, 20)
        s_goal = (40, 10)
        ...
        self.plotting = plotting.Plotting(s_start, s_goal, cost_map)
        self.utils = utils.Utils(cost_map)
    ...
    def on_press(self, event):
        ...
        if x < 0 or x > 50 or y < 0 or y > 30:
            ...
            self.utils.update_obs(self.obs_rectangle, self.obs_circle)
            self.InvalidateNodes()

            if self.is_path_invalid():
                print("Path is Replanning ...")
                print(self.replanning())
    ...
    def run(self):
        path = self.planning()
        print("PATH", path)
        self.plotting.animation(self.vertex, path, "Dynamic_RRT", True)
